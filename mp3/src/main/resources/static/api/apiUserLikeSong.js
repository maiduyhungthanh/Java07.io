const API_UserById = "http://localhost:1993/api/userlikesong";
...
const getListSong = async () => {
    let res = await axios.get(`http://localhost:1993/api/userlikesong/${id}`);
    myPlayList = res.data;
    ...
    for (let i = 0; i < myPlayList.length; i++) {
      ...
      <td class="btn_del" title="${s.id}"><img src="/img/svg/delete.png" </td>
      </tr>
		`
        singer_name.splice(0, singer_name.length);
    }
    songAll.innerHTML = html
        // delete bài hat trong DS yeu thich
        const btn_del = document.querySelectorAll(".btn_del");
        for (let i = 0; i < btn_del.length; i++) {
          var id_song = btn_del[i].title
          btn_del[i].addEventListener("click", async function () {
          let res = await axios.delete(`http://localhost:1993/api/user/delete/${id_song}/${id}`)
          alert("đã xóa thành công")
          window.location.href = `http://localhost:1993/userlikesong|${id}`
      })
}
