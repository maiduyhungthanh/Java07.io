...
const getListSong = async () => {
  ...
  for (let i = 0; i < myPlayList.length; i++) {
    ...
    html += `
		...
		<tr>
		  ...
		  <td class="tim" title="${s.id}"><img src="img/svg/timxanh.png"></td>
		</tr>
		`
  ...
  songAll.innerHTML = html
      ...
      // chuyển màu tim yêu thích
      const tim = document.querySelectorAll(".tim") 
      // const getListTim = async () => {
      for (let z = 0; z < myPlayList.length; z++) {         
      tim[z].addEventListener("click",async function(){
        let res = await axios.post(`http://localhost:1993/api/songByUser`,
        {
          id_song:tim[z].title,
          id_user:id_user
        })
          alert("đã thêm vào danh sách yêu thích")     
        if(tim[z].innerHTML==`<img src="img/svg/timxanh.png">`){
          tim[z].innerHTML=`<img src="img/svg/timdo.png">`
        }       
      })
    }
  // }
